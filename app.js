...
/* OAS OMR Marker (GitHub Pages, client-side)
   - Default template auto-loads from data/template.pdf (if present)
   - Upload teacher key + student PDFs (multiple files allowed)
   - Outputs results.csv (student_number, score) + optional annotated.pdf
*/
...
(() => {
  ...
  const cvReady = new Promise((resolve) => {
    const poll = () => {
      if (typeof cv !== 'undefined' && cv && cv.Mat) {
        // Some builds require onRuntimeInitialized
        if (typeof cv.onRuntimeInitialized === 'function') {
          const prev = cv.onRuntimeInitialized;
          cv.onRuntimeInitialized = () => { try { prev(); } catch (_) {} resolve(); };
        } else {
          resolve();
        }
      } else {
  ...
  function buildLayoutFromTemplate(t1RGBA, t2RGBA) {
    const cAns = detectCirclesInRegion(t1RGBA, CFG.p1_answers_region);
    ...
    for (let row_i=0; row_i<centersY.length; row_i++){
      ...
      for (let block_i=0; block_i<top4.length; block_i++){
        ...
        const qnum = block_i*10 + (row_i+1);
        questions[String(qnum)] = opts.map((opt, j) => ({ opt, x: g[j][0], y: g[j][1], r: g[j][2] }));
      }
    }

    const cSN = detectCirclesInRegion(t1RGBA, CFG.p1_student_number_region);
    ...
    const cP2 = detectCirclesInRegion(t2RGBA, CFG.p2_first_column_region);
  ...
  function readAnswers(alignedRGBA, layout, qnums, minFill) {
    ...
    for (const q of qnums) {
      ...
      answers[q] = pick(scores, minFill, CFG.multi_delta).choice;
    }
    gray.delete();
    return answers;
  }
  ...
  function readStudentNumber(alignedRGBA, layout) {
    ...
    for (const col of layout.studentNumber.columns) {
      ...
      digits.push(pick(scores, CFG.digit_min_fill, CFG.multi_delta).choice ?? '');
    }
    gray.delete();
  ...
  function maybeEnableRun() {
    const ok = !!(state.template.bytes && state.key.answers && state.students.bytes.length);
    btnRun.disabled = !ok;
  }

  async function ensureReady() {
    if (state.ready.cv) return;
    log('Loading OpenCV.js (first time may take a few seconds)...');
    await cvReady;
    state.ready.cv = true;
    log('OpenCV.js ready.');
  }

  async function loadTemplate(bytes, label) {
    await ensureReady();
    clearOutputs();
    // free old mats
    ...
    state.layout = null;
    state.key.answers = null; // key must be reread if template changes

    log(`Rendering template: ${label}`);
    ...
    log(`Template rendered (${m1.cols}x${m1.rows}). Building ORB features...`);
    state.template.orb1 = buildOrbTemplate(m1);
    state.template.orb2 = buildOrbTemplate(m2);
    log('Template ready. Now upload teacher key.');
    maybeEnableRun();
  }
  ...
  async function buildLayoutIfNeeded() {
    ...
    log(`Layout ready. Questions mapped: ${Object.keys(state.layout.questions).length}.`);
  }

  async function readKey(bytes) {
    await ensureReady();
    clearOutputs();
    if (!state.template.bytes) throw new Error('Template not loaded yet.');
    ...
    const key1 = readAnswers(a1, state.layout, qnums.filter(q=>q<=40), CFG.answer_min_fill);
    const key2 = readAnswers(a2, state.layout, qnums.filter(q=>q>=CFG.q2_start), CFG.answer_min_fill);
    ...
    if (bad.length) {
      ...
      throw new Error(`Teacher key has blank/ambiguous answers at: ${bad.join(', ')}. (Did you upload the blank template by mistake?)`);
    }

    m1.delete(); m2.delete(); a1.delete(); a2.delete();
    state.key.bytes = bytes;
    state.key.answers = key;
    log('Teacher key read successfully. Now select student PDFs.');
    maybeEnableRun();
  }
  ...
  async function loadStudentsFromFiles(fileList) {
    ...
    log(`Loaded ${state.students.files.length} student PDFs.`);
    maybeEnableRun();
  }

  async function markOneStudent(bytes, label) {
    const c1 = await renderPdfPageToCanvas(bytes, 0, CFG.dpiScale);
    const c2 = await renderPdfPageToCanvas(bytes, 1, CFG.dpiScale);
    ...
    const qnums = [...Array(40)].map((_,i)=>i+1).concat([...Array(CFG.q2_count)].map((_,i)=>CFG.q2_start+i));
    const ans1 = readAnswers(a1, state.layout, qnums.filter(q=>q<=40), CFG.answer_min_fill);
    const ans2 = readAnswers(a2, state.layout, qnums.filter(q=>q>=CFG.q2_start), CFG.answer_min_fill);
    ...
    m1.delete(); m2.delete(); a1.delete(); a2.delete();
    return { student_number: studentNumber, score, label, ann1, ann2 };
  }
  ...
  async function runMarking() {
    ...
    try {
      ...
      if (!state.key.answers) throw new Error('Teacher key not loaded.');
      if (!state.students.bytes.length) throw new Error('No student PDFs selected.');

      await buildLayoutIfNeeded();

      const results = [];
      const total = state.students.bytes.length;
      log(`Marking ${total} student PDFs...`);

      for (let i=0;i<total;i++){
        setProgress((i/total)*100);
        const label = state.students.files[i].name || `student_${i+1}.pdf`;
  ...
  btnReset.addEventListener('click', () => {
    ...
    setProgress(0);
    logEl.textContent = '';
    // Keep template loaded; just clear key/students
    ...
    fileKey.value = '';
    fileStudents.value = '';
    log('Reset: cleared key + students. (Template kept.)');
    maybeEnableRun();
  });
  ...
  fileTemplate.addEventListener('change', async (e) => {
    ...
    try {
      ...
      await loadTemplate(bytes, f.name);
    } catch (err) {
  ...
  btnLoadTemplateFromManifest.addEventListener('click', async () => {
    try {
      const bytes = await fetchBytes('data/template.pdf');
      await loadTemplate(bytes, 'data/template.pdf');
    } catch (err) {
      log(`ERROR: ${err.message || err} (Is data/template.pdf in your repo?)`);
    }
  });
  ...
  // Startup: auto-load default template if present
  (async () => {
    clearOutputs();
    setProgress(0);
    log('Tip: You only need to upload the teacher key + student PDFs.');
    try {
      await ensureReady();
      const bytes = await fetchBytes('data/template.pdf');
      await loadTemplate(bytes, 'data/template.pdf (auto)');
      log('Default template auto-loaded.');
    } catch (_) {
      log('No default template found in repo. Upload a template to begin.');
    }
    maybeEnableRun();
  })();
})();
